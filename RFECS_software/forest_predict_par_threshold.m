function forest_predict_par_threshold(forest_all,nvar,ntrees,len_var,mod_vec,extn,peak_filt_dist,input_path,output_path,chr_set, threshold)
% enhancer predictions using p300 vs TSS-bg

%use -XY when using GUI
...
for x=1:length(chr_set)
        ...
        for k=1:len_var
            ...
            for i=1:ntrees %for i=1:ntrees %%%CHANGED parfor to for
    ...
    index_sel=find(mean_bg_all>threshold); %mean_bg_all 2492487 x 1
    ...
    for i=1:length(patches2) %109897
        i
        abc=[];
        for j=patches2(i,1):patches2(i,2) %jotain menee pieleen
            j
