...
if(length(prob_dist)<=peak_dist)
    ...
    vectors(1).start=start-peak_dist+1;
    vectors(2).start=start+second_vector_start-1;
    ...
    while(~isempty(vectors)) %1
       ...
       peak_dist_length= find((extractfield(vectors, 'length')<=peak_dist) ==1);
       if(~isempty(peak_dist_length))
           for i=1:length(peak_dist_length)
              % short='short';
               ...
               indiv_enhancers_pks=[indiv_enhancers_pks pks];
               indiv_enhancers_locs=[indiv_enhancers_locs vectors(peak_dist_length(i)).start+locs-2];
       ...
       if(~isempty(peak_dist_length))
           ...
           for i=1:length(peak_dist_length)
               ...
               [pks locs]=max( vectors(peak_dist_length(i)).vector );
               indiv_enhancers_pks=[indiv_enhancers_pks pks];
               indiv_enhancers_locs=[indiv_enhancers_locs vectors(peak_dist_length(i)).start+locs-length(peak_dist_length)+i-1];
               ...
               vectors_temp(1).start=vectors(peak_dist_length(i)).start+1;
               vectors_temp(2).start=vectors(peak_dist_length(i)).start+second_vector_start-1;
